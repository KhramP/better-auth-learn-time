...
export function PasskeyManagement({ passkeys }: { passkeys: Passkey[] }) {
  ...
  function handleDeletePasskey(passkeyId: string) {
    return authClient.passkey.deletePasskey({ id: passkeyId }, { onSuccess: () => router.refresh() });
  }
      ...
      {passkeys.length === 0 ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Add your first passkey for secure, passwordless authentication.</CardDescription>
          </CardHeader>
        </Card>
      ) : (
