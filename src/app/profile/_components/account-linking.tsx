    ...
    <div className="space-y-6">
      <div className="space-y-2">
        ...
        {currentAccounts.length === 0 ? (
          <Card>
            <CardContent className="py-8 text-center text-secondary-muted">No linked accounts found</CardContent>
          </Card>
        ) : (
...
function AccountCard({ provider, account }: { provider: string; account?: Account }) {
  ...
  const providerDetails = SUPPORTED_OAUTH_PROVIDERS_DETAILS[provider as SupportedOAuthProvider] ?? {
    name: provider,
    Icon: Shield,
  };
  ...
  function unlinkAccount() {
    ...
    return authClient.unlinkAccount(
      { accountId: account.accountId, providerId: provider },
      {
        onSuccess: () => {
          router.refresh();
        },
