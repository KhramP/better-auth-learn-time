...
async function LinkedAccountTab() {
...
async function SessionTab({ currentSessionToken }: { currentSessionToken: string }) {
  const session = await auth.api.listSessions({ headers: await headers() });

  return (
    <Card>
      <CardContent>
        <SessionManagement session={session} currentSessionToken={currentSessionToken} />
      </CardContent>
    </Card>
  );
}
      ...
      {hasPasswordAccount ? (
        <Card>
          <CardHeader>
            ...
            <CardDescription>Change your password for improved security.</CardDescription>
          </CardHeader>
          <CardContent>
            <ChangePasswordForm />
          </CardContent>
